...
#   kernelspec:
#     display_name: asf-lifetime-cost-model (3.13.1)
#     language: python
#     name: python3
# ---
...
# %%
sns.histplot(data=band_clean, x="_dist")
...
def _compute_fixed_bandwidth(values: pd.Series, k: int = 2, eps: float = 1.0000001) -> float:
    """Compute a kernel bandwidth given an array of data."""
    x = values.sort_values(ascending=True).values
    dists = numpy.abs(x[:, numpy.newaxis] - x[numpy.newaxis, :])
    sorted_dists = numpy.sort(dists, axis=1)
    d_ik = sorted_dists[:, k]
    return numpy.max(d_ik) * eps
...
def gaussian_weights(z: pd.Series) -> pd.Series:
    ...
    w = (2 * numpy.pi) ** (-1 / 2) * numpy.exp(-(z**2) / 2)
    return w / w.sum() * z.shape[0]


# %%
# Create a bandwidth normalised distance variable.
band_clean["_dist_z"] = band_clean["_dist"] / _compute_fixed_bandwidth(band_clean["cost"])
# Get weights
...
# %%
band_clean.groupby("PROPERTY_TYPE")["gaussian_weight"].sum().to_frame().assign(
    prop=lambda df: df["gaussian_weight"] / df["gaussian_weight"].sum()
)

# %%
band_clean.groupby("PROPERTY_TYPE")["quartic_weight"].sum().to_frame().assign(
    prop=lambda df: df["quartic_weight"] / df["quartic_weight"].sum()
)
...
# %%
# Create a bandwidth normalised distance variable.
band_clean["_dist_z"] = band_clean["_dist"] / _compute_fixed_bandwidth(band_clean["heat_demand"])
# Get weights
band_clean["quartic_weight"] = quartic_weights(band_clean["_dist_z"])
band_clean["gaussian_weight"] = gaussian_weights(band_clean["_dist_z"])
