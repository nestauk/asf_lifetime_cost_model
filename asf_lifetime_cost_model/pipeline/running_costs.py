...
def _get_property_heat_demand(archetype_number: int) -> float:
    """
    Helper function to extract annual heat demand value for a given property archetype.

    Args:
        archetype_number (int): Number of property archetype, valid arguments are [1, 8].
...
def _get_wholesale_price_projection_series(
    fuel_type: str, projection_scenario: str
) -> pd.DataFrame:
    """
    Helper function to get a slice of the DESNZ wholesale price projection dataframe by filtering on given fuel type (electricity or gas) and projection scenario name.
...
def _create_energy_cost_time_series(
    ...
    """
    Creates two time series of total energy costs in each year of operation (one for unit costs and one for standing charges),
...
def compute_running_costs(
    ...
    Args:
        ...
        heating_system_efficiency (float): Assumed efficiency of heating system, expressed as a ratio.
            ...
            3.0 means the system produces 3 units of heat per 1 unit of input energy..
